...
app.post("/api/endpoint/close", async (req, res) => {
  const data = req.body;
...
app.post("/api/endpoint/signup", async (req, res) => {
  ...
  const user = new User({ ...data, transactions: [], loanRequests: [] });
  user.save();
  // console.log(data);
});
...
async function closeAccount(username, pin) {
  query = { username: username, password: pein };
